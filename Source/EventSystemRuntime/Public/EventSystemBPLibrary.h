...
public:
	...
	UFUNCTION(BlueprintCallable, Category = "EventSystem", meta = (HidePin = "WorldContext", DefaultToSelf = "WorldContext"))
	static void UnListenEvent(const UObject* WorldContext, const FEventHandle& Handle);

	UFUNCTION(BlueprintCallable, meta = (CallableWithoutWorldContext, HidePin = "Listener", DefaultToSelf = "Listener"), Category = "EventSystem")
	static void UnListenEvents(UObject* Listener); // FIX (blowpunch)
