...
void UEventsK2Node_EventBase::PinDefaultValueChanged(UEdGraphPin* Pin)
{
	if (Pin == GetEventPin())
	{
		...
		for (int32 PinIdx = Pins.Num() - 1; PinIdx > 0; PinIdx--)
		{
			...
			if (DefaultPins.Contains(Pins[PinIdx]))
			{
				continue;
			}
...
{
	DefaultPins.Add(CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Execute));
	DefaultPins.Add(CreatePin(EGPD_Output, UEdGraphSchema_K2::PC_Exec, UEdGraphSchema_K2::PN_Then));
	CreateReturnEventHandlePin();
	DefaultPins.Add(CreatePin(EGPD_Input, UEdGraphSchema_K2::PC_Object, UEdGraphSchema_K2::PSC_Self, UEdGraphSchema_K2::PN_Self));
	CreateSelectionPin();
	CreateOutEventPin();
