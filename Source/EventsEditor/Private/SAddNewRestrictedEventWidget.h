...
class SAddNewRestrictedEventWidget : public SCompoundWidget
{
public:

	DECLARE_DELEGATE_ThreeParams(FOnRestrictedEventAdded, const FString& /*TagName*/, const FString& /*TagComment*/, const FName& /*TagSource*/);

	SLATE_BEGIN_ARGS(SAddNewRestrictedEventWidget)
		: _NewRestrictedTagName(TEXT(""))
		{}
		SLATE_EVENT( FOnRestrictedEventAdded, OnRestrictedEventAdded )	// Callback for when a new tag is added	
		SLATE_ARGUMENT( FString, NewRestrictedTagName ) // String that will initially populate the New Tag Name field
	SLATE_END_ARGS();

	virtual ~SAddNewRestrictedEventWidget();
...
private:
	...
	FOnRestrictedEventAdded OnRestrictedEventAdded;
	...
	TSharedPtr<SNotificationItem> AddRestrictedEventDialog;
};
